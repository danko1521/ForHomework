...
class VKfinder:
    ...
    def write_msg(self, user_id, message, photos=None):
        ...
        if photos == None:
            self.vk1.method('messages.send',
                            {'user_id': user_id, 'message': message, 'random_id': random.randrange(10 ** 7)})
        else:
            self.vk1.method('messages.send', {'peer_id': user_id, 'message': message, 'attachment': photos,
                    ...
                    if request in ['хочу', 'найди', 'да', 'валяй', 'найди мне пару']:
                        ...
                        if not self.db_status:
                            self.write_msg(event.user_id,
                                           f'Упс! Что-то пошло не так и наша база данных недоступна. Результаты поиска могут повторяться.')
                        ...
                        try:
                            ...
                            message = 'В каком году ты родилась?' if vk_user_params[
                                                                         1] == 1 else 'В каком году ты родился?'
                        ...
                        if self.db_status:
                            if not self.db.find_in_users(vk_user_params[0]):
                                self.db.add_user(vk_user_params[0], vk_user_params[2], vk_user_params[1],
                                                 vk_user_params[3])

                        vk_user_params = [vk_user_params[0], 1 if vk_user_params[1] == 2 else 2, vk_user_params[2],
                                          vk_user_params[3]]
                        ...
                        if photos == 'Список пуст! Не удалось никого найти.':
                            ...
                            if 'relation' in found_info['response'][0].keys():
                                ...
                                found_status = 9
                            self.write_msg(event.user_id,
                                           f'Знакомься, это {found_firstname} (https://vk.com/id{id_vk_found})', photos)
            ...
            if event.type == VkEventType.MESSAGE_NEW:
                if event.to_me:
                    ...
                    if request in ['нет', 'не похоже']:
                        ...
                        self.write_msg(event.user_id, f'попробуй еще разок)')
                        self.write_msg(event.user_id, f'Если хочешь повторить поиск, напиши: Привет')
                        break
